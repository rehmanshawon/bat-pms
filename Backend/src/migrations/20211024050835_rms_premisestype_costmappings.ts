...
export async function up(knex: Knex): Promise<void> {
  return knex.schema.createTable('rms_premisestype_costmappings', (table) => {
...
export async function down(knex: Knex): Promise<void> {
  await knex.schema
    .hasTable('rms_premisestype_costmappings')
    .then(async function (exists) {
      if (exists) {
        await knex.schema.alterTable(
          'rms_premisestype_costmappings',
          (table) => {
        ...
        return knex.schema.dropTable('rms_premisestype_costmappings');
      }
    });
}
