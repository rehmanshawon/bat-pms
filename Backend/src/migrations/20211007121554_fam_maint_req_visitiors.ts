...
export async function up(knex: Knex): Promise<void> {
  return knex.schema.createTable('fam_maint_req_visitiors', (table) => {
    table.increments('maint_req_visitior_id').notNullable();
    table
    ...
    table.string('other_visitor_info', 200).nullable().comment('move up');
...
export async function down(knex: Knex): Promise<void> {
  await knex.schema
    .hasTable('fam_maint_req_visitiors')
    .then(async function (exists) {
      if (exists) {
        await knex.schema.alterTable('fam_maint_req_visitiors', (table) => {
        ...
        return knex.schema.dropTable('fam_maint_req_visitiors');
      }
    });
}
