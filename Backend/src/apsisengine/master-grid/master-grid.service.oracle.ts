...
export class MasterGridService {
  ...
  async gridData(req_data, user_data) {
    ...
    console.log('datatable_query', datatable_query);

    let rows = [];
    if (grid_info.default_data) {
      rows = await this.knex.raw(datatable_query);
    } else {
      if (req_data.search_data.length > 0) {
        rows = await this.knex.raw(datatable_query);
      }
    }
    ...
    return req_data['page'] ? result_array : data_array;
  }
  ...
  async dropDownQueryGenerator(req_data, user_data) {
    ...
    if (req_data['search_columns']) {
      ...
      if (groupBy) {
        const groupByStr = groupBy.replace('GROUP BY', '');
        search_columns = groupByStr.split(',');
      }
      ...
      // const all_field =
      //   typeof all_field_query[0] !== 'undefined' ? all_field_query[0] : [];
    ...
    if (table_search && search_columns.length > 0) {
      for (const [i, column] of Object.entries(search_columns)) {
        ...
        if (i == '0') {
          having_from_search +=
            'regexp_like(' + columnLower + ",'" + table_search + "', 'i')";
        } else {
          having_from_search +=
            ' OR ' +
            'regexp_like(' +
    ...
    if (having_from_search.trim() != '') {
      having_from_search = having_from_search.trim().replace('AND', '');
      sqlcondition = sqlcondition + ' AND (' + having_from_search + ')';
    }

    // if (having_from_search) {
    //   sqlcondition = ' AND (' + having_from_search + ')';
    // }
  ...
  async customSearchFormatter(custom_search_con) {
    ...
    for (const [name, value] of Object.entries(custom_search_con)) {
      if (value) {
        ...
        const col_name2 = col_name1.replace(/[[]]/g, '');
        let col_name_array: any;
        let col_name: string;
        if (col_name2.indexOf('.') !== -1) {
          col_name_array = col_name2.split('.');
          col_name = '"' + col_name_array[0] + '"."' + col_name_array[1] + '"';
        } else {
          col_name = '"' + col_name2 + '"';
        }
        if (name == 'tagg') {
  ...
  async sqlQueryConcat(data, extra_con, user_data) {
    ...
    if (
      ...
      const extraCondition = await this.planeExtraCondition(
        extra_con['extra_condition'],
      );
      if (where_condition) {
        where_condition += ' AND ' + extraCondition;
      } else {
        where_condition += ' WHERE ' + extraCondition;
      }
    }
    ...
    if (extra_con.hasOwnProperty('slug') && extra_con.slug !== '') {
      ...
      if (sqlgroupby) {
        dgroupby =
          sqlgroupby + ',"sys_delegation_columns"."delegation_version"';
      }
      ...
      const sqlsource_for_del =
        ...
        '" AND "sys_delegation_columns"."table_name" = ' +
        "'" +
        id_logic.ref_db_table_name +
        "'";
      ...
      const sql_query =
        sqltext +
        ',"sys_delegation_columns"."delegation_version" AS "hidden_delegation_version"' +
        ' ' +
        sqlsource_for_del +
        ' ' +
        where_condition +
      ...
      same_sort_sqltext +=
        ' LEFT JOIN "sys_delegation_conf" ON "sys_delegation_conf"."delegation_for"="sys_delegation_columns"."delegation_for" AND "sys_delegation_conf"."ref_event_id"="sys_delegation_columns"."delegation_ref_event_id" AND "sys_delegation_conf"."delegation_version"="sys_delegation_columns"."delegation_version" AND "sys_delegation_conf"."step_number"="sys_delegation_columns"."delegation_step" AND "sys_delegation_conf"."same_sort"=1 AND "sys_delegation_conf"."user_id"=' +
        user_data.user_id;
  ...
  async doubleQoutationAdd(condition) {
    return condition;
  }
  ...
  async gridtitle(req_data, user_data) {
    ...
    req_data['having_condition'] = having_condition;
    const query = await this.sqlQueryConcat(req_data, extra_data, user_data);
    //console.log('jak'+query);
    const rows = await this.knex.raw(query);
    //console.log('jak'+rows.length);
    //return 'ajkkkkk';
    const total_rows = grid_info.default_data ? rows.length : 0;
    const linkable_column = await this.dataLinkCheck(grid_info.data_link);
    //console.log(linkable_column);
    ...
    //   rows.length > 0
    //     ? Object.keys(rows[0])
    //     : await this.commonHelper.columnArray(grid_info.sql_select);

    const fields =
      rows.length > 0 ? Object.keys(rows[0]) : await this.columnArray(query);

    for (const key of fields) {
      const flag = key.split('_');
      const newTitle = key.replace(/apsisamt_|apsisqty_|hidden_/g, '');
      ...
      if (flag[0] == 'hidden') {
        ...
        if (grid_info.master_column_title) {
          ...
          build_array['sortable'] = true;
          build_array['text_align'] = flag[0] == 'apsisamt' ? 'right' : 'left';
          build_array['field_type'] =
            flag[0] == 'apsisamt' || flag[0] == 'apsisqty' ? 'number' : 'text';
        }
        build_array['dataIndex'] = key;
        build_array['key'] = key;
        build_array['link'] = linkable_column.includes(key) ? true : false;
  ...
  async columnArray(query) {
    const sql = `SELECT
    *
    FROM
    ( SELECT 1 FROM DUAL )
    LEFT JOIN (${query} FETCH FIRST 1 ROWS ONLY
      ) tbl ON 1 = 1`;
    //console.log('jak'+sql);
    const sql_result = await this.knex.raw(sql);
    const colArray = [];
    let sl = 0;
    for (const [k, v] of Object.entries(sql_result[0])) {
      if (sl > 0) {
        colArray.push(k.replace(/['"]+/g, ''));
      }
      sl++;
    }
    return colArray;
  ...
  async exportXlsx(req_data, user_data) {
    ...
    const bindKnex = this.knex;
    const result = await generateXlsx(payload, user_data, bindKnex);
  ...
  WHERE
    ...
    AND cons.owner = '${database_user.toUpperCase()}'
GROUP BY(
    cols.table_name,
    cols.column_name
)
  ...
  async mastergridCreate(mastergridData: CreateMastergridDto, userPayload) {
    const payload: any = mastergridData;
    console.log(payload);
  ...
  async getMastergridData(id: number, userPayload) {
    ...
    // data['is_selectable'] = data.is_selectable = 1? 'Yes': 'No';
    // data['enable_form'] = data.enable_form = 1? 'Yes': 'No';
    // data['export_csv'] = data.export_csv = 1? 'Yes': 'No';
    // data['export_excel'] = data.export_excel = 1? 'Yes': 'No';
    // data['export_pdf'] =  data.export_pdf = 1? 'Yes': 'No';
    // data['export_printing'] = data.export_printing = 1? 'Yes': 'No';
    // data['grid_checkbox'] = data.grid_checkbox = 1? 'Yes' : 'No';
    // data['grid_serial'] = data.grid_serial = 1? 'Yes': 'No';
    // data['tagg_html'] = data.tagg_html = 1? 'Yes' : 'No';
    // data['client_side'] = data.client_side = 1? 'Yes': 'No';
    return {
      data: data,
  ...
  async remove(data: DeleteMasterGridDto, userpayload) {
    ...
    const time = this.commonFunction.cmnDatetime();
    const result = await this.knex('sys_master_grids')
