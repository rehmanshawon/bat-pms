  ...
  ({ editId, gridRef, handleCancel, ...props }, ref) => {
    ...
    const storeData = async (values) => {
      ...
      await fetchWrapper
        ...
        .then((response) => {
          if (!response.error) {
            swalSuccess("Role Created Successfully").then(() => {
              setDefaultValues({});
              gridRef.current.fetchInfo();
            });
          } else {
            swalError("Role couldn't be created");
          }
        })
        .finally(() => {
    ...
    const updateData = async (values) => {
      const data = { ...defaultValues, ...values };
      await fetchWrapper
        .patch("role/" + editId, data)
        .then((response) => {
          if (!response.error) {
            swalSuccess("Role Updated Successfully").then(() => {
              setDefaultValues({});
              gridRef.current.fetchInfo();
            });
            //gridRef.current.setState({ selectedRowKeys: [] });
          } else {
            swalError("Role couldn't be updated");
          }
        })
