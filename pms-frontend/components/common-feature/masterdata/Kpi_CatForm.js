  ...
  ({ editId, gridRef, handleCancel, ...props }, ref) => {
    ...
    const storeData = async (values) => {
      ...
      await fetchWrapper
        ...
        .then((response) => {
          if (!response.error) {
            swalSuccess("Kpi_Cat Created Successfully").then(() => {
              setDefaultValues({});
              gridRef.current.fetchInfo();
            });
          } else {
            swalError("Kpi_Cat couldn't be created");
          }
        })
        .finally(() => {
    ...
    const updateData = async (values) => {
      const data = { ...defaultValues, ...values };
      await fetchWrapper
        .patch("kpi_cat/" + editId, data)
        .then((response) => {
          if (!response.error) {
            swalSuccess("Kpi_Cat Updated Successfully").then(() => {
              setDefaultValues({});
              gridRef.current.fetchInfo();
            });
            //gridRef.current.setState({ selectedRowKeys: [] });
          } else {
            swalError("Kpi_Cat couldn't be updated");
          }
        })
