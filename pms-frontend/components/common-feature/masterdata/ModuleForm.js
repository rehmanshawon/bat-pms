...
const ModuleForm = forwardRef(
  ({ editId, gridRef, handleCancel, ...props }, ref) => {
    ...
    const storeData = async (values) => {
      ...
      await fetchWrapper
        .post("bat_modules", data)
        .then((response) => {
          if (!response.error) {
            swalSuccess("Module Created Successfully").then(() => {
              setDefaultValues({});
              gridRef.current.fetchInfo();
            });
          } else {
            swalError("Module couldn't be created");
          }
        })
        .finally(() => {
    ...
    const updateData = async (values) => {
      const data = { ...defaultValues, ...values };
      await fetchWrapper
        .patch("bat_modules/" + editId, data)
        .then((response) => {
          if (!response.error) {
            swalSuccess("Module Updated Successfully").then(() => {
              setDefaultValues({});
              gridRef.current.fetchInfo();
            });
            //gridRef.current.setState({ selectedRowKeys: [] });
          } else {
            swalError("Module couldn't be updated");
          }
        })
