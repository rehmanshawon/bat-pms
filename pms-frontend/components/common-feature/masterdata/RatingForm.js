  ...
  ({ editId, gridRef, handleCancel, ...props }, ref) => {
    ...
    const getFields = async () => {
      let response = await fetchWrapper.post("masterform/getformdata", {
        form_slug: "rating_definition_form",
      });
      if (response.data) {
    ...
    const storeData = async (values) => {
      ...
      await fetchWrapper
        ...
        .then((response) => {
          if (!response.error) {
            swalSuccess("Rating Created Successfully").then(() => {
              setDefaultValues({});
              gridRef.current.fetchInfo();
            });
          } else {
            swalError("Rating couldn't be created");
          }
        })
        .finally(() => {
    ...
    const updateData = async (values) => {
      const data = { ...defaultValues, ...values };
      await fetchWrapper
        .patch("rating/" + editId, data)
        .then((response) => {
          if (!response.error) {
            swalSuccess("Rating Updated Successfully").then(() => {
              setDefaultValues({});
              gridRef.current.fetchInfo();
            });
            //gridRef.current.setState({ selectedRowKeys: [] });
          } else {
            swalError("Rating couldn't be updated");
          }
        })
