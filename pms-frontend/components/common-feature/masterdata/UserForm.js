  ...
  ({ editId, gridRef, handleCancel, ...props }, ref) => {
    ...
    const storeData = async (values) => {
      ...
      await fetchWrapper
        ...
        .then((response) => {
          if (!response.error) {
            swalSuccess("User Created Successfully").then(() => {
              setDefaultValues({});
              gridRef.current.fetchInfo();
            });
          } else {
            swalError("User couldn't be created");
          }
        })
        .finally(() => {
    ...
    const updateData = async (values) => {
      const data = { ...defaultValues, ...values };
      await fetchWrapper
        .patch("user/" + editId, data)
        .then((response) => {
          if (!response.error) {
            swalSuccess("User Updated Successfully").then(() => {
              setDefaultValues({});
              gridRef.current.fetchInfo();
            });
            //gridRef.current.setState({ selectedRowKeys: [] });
          } else {
            swalError("User couldn't be updated");
          }
        })
