...
const DesignationForm = forwardRef(
  ({ editId, gridRef, handleCancel, ...props }, ref) => {
    ...
    const storeData = async (values) => {
      ...
      await fetchWrapper
        ...
        .then((response) => {
          if (!response.error) {
            swalSuccess("Designation Created Successfully").then(() => {
              setDefaultValues({});
              gridRef.current.fetchInfo();
            });
          } else {
            swalError("Designation couldn't be created");
          }
        })
        .finally(() => {
    ...
    const updateData = async (values) => {
      const data = { ...defaultValues, ...values };
      await fetchWrapper
        .patch("designation/" + editId, data)
        .then((response) => {
          if (!response.error) {
            swalSuccess("Designation Updated Successfully").then(() => {
              setDefaultValues({});
              gridRef.current.fetchInfo();
            });
            //gridRef.current.setState({ selectedRowKeys: [] });
          } else {
            swalError("Designation couldn't be updated");
          }
        })
    ...
    return (
      <>
        <Form
          form={form}
          name="designation_form_controls"
          layout="horizontal"
          onFinish={onFinish}
