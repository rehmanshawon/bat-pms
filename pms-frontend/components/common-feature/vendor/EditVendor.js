...
const EditVendor = () => {
  ...
  const makeBankData = (data) => {
    data.map((item, index) => {
      setInitialValue((prev) => ({
        ...prev,
        [`bank_id[${index}]`]: item.bank_id,
      }));
      setInitialValue((prev) => ({
        ...prev,
        [`branch_id[${index}]`]: item.branch_id,
      }));
      setInitialValue((prev) => ({
        ...prev,
        [`account_type[${index}]`]: item.account_type,
      }));
      setInitialValue((prev) => ({
        ...prev,
        [`account_no[${index}]`]: item.account_no,
      }));
    });
  };
  useEffect(async () => {
    await fetchWrapper.get(`vendor/${router.query.code}`).then((res) => {
      setEditData(res.data);
      setBankData(res.data["bank_data"]);
      makeBankData(res.data["bank_data"]);
    });
  }, []);
  ...
  const handleChange = (e, index) => {
    ...
    if (name == `bank_id[${index}]`) {
      bank_Data[index].bank_id = value;
      setBankinfo(bank_Data);
      setBankId(value);
    } else if (name == `branch_id[${index}]`) {
      bank_Data[index].branch_id = value;
      setBankinfo(bank_Data);
    } else if (name == `account_type[${index}]`) {
      bank_Data[index].account_type = value;
      setBankinfo(bank_Data);
    } else if (name == `account_no[${index}]`) {
      bank_Data[index].account_no = value;
      setBankinfo(bank_Data);
    ...
    <div className="main-wrap">
      <div className="card card-xl-stretch">
        <div className="card-header border-0 p-4 d-flex justify-content-between">
          <h3 className="card-title fw-bolder text-dark mb-0"></h3>
        </div>
