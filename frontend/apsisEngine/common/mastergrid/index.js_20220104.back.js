...
export const MasterGrid = React.forwardRef(
  (
    ...
    const [is_selectable, setSelectable] = useState(1);
    ...
    const rowSelection = {
      getCheckboxProps: (record) => ({
        disabled: record.key == "PR-21-12-000028",
        key: record.key,
      }),
    ...
    const fetchTitle = async () => {
      ...
      await fetchWrapper
        ...
        .then(async (res) => {
          ...
          // if (extraButtons) {
          //   const withExtraButtons = [...buttons, ...extraButtons];
          //   //set with extra buttons
          //   setInitialButtons(withExtraButtons);
          //   setButtonData(withExtraButtons);
          // } else {
          //   //set buttons
          ...
          const filteredColumn = generatedColumn.map((column) => {
            ...
            const render =
              ...
              column.dataIndex.includes("_at")
                ? (t, r) => moment(t).format("D-M-YY HH:mm")
                : (t, r) => t;
    ...
    useEffect(async () => {
      if (griData.length == 0 && primaryKeyField && !is_clientSide) {
        await fetchData();
      }
        ...
        <div className="master_grid">
          ...
          <Table
            className={`grid-table ${
              is_selectable == 0 ? "disable_select" : ""
            }`}
            rowSelection={{
              type: selectionType,
              selectedRowKeys: selectedIds,
